...
use OCA\OpenCase\Service\SamlAssertionValidator;
...
use OCA\OpenCase\Service\RedirectValidator;
...
class SamlController extends Controller {
	...
	public function login(): RedirectResponse {
		...
		// redirect_url is handed to the IdP as RelayState and comes back to the
		// ACS endpoint as the post-login destination, so it is pinned to this
		// instance here rather than trusted on the way back.
		$returnTo = $this->safeReturnTo($params['redirect_url'] ?? null);
	...
	public function acs(): Response {
		...
		// The AuthnRequest we sent, so the response can be tied back to it.
		$expectedRequestId = $this->session->get('opencase.request_id') ?: null;

		try {
			$responseDoc = $this->loadXmlStrict($samlResponseXml, 'SAML response');

			// Nothing in this message may be trusted until the IdP's signature
			// over it has been verified. The assertion is encrypted to our
			// public key, but that key is published in our own metadata — so
			// encryption alone would let anyone mint an assertion naming any
			// user with any privileges.
			$responseWasSigned = $this->samlAssertionValidator->validateResponse($responseDoc, $expectedRequestId);

			$decryptedAssertion = $this->decryptSamlAssertion($samlResponseXml);

			$assertionDoc = $this->loadXmlStrict($decryptedAssertion, 'SAML assertion');
			$assertionInfo = $this->samlAssertionValidator->validateAssertion(
				$assertionDoc,
				$expectedRequestId,
				$responseWasSigned,
			);
		} catch (SamlValidationException $e) {
			$this->traceLogger->trace('saml_validation_failed', [
				'error' => $e->getMessage(),
			]);
			$this->logger->warning('OpenCase: rejected SAML response — {msg}', [
				'app' => 'opencase',
				'msg' => $e->getMessage(),
			...
			$this->traceLogger->error('saml_processing_failed', $e);
			$this->logger->warning('OpenCase: failed to process SAML response — {msg}', [
				'app' => 'opencase',
				'msg' => $e->getMessage(),
			]);
			return $this->showNoAccessPage();
		...
		// Replay protection, keyed on the assertion's own ID and held for as
		// long as the assertion claims to be valid (a hash of the encoded
		// response would miss a re-encrypted replay of the same assertion).
		$replayKey = 'saml_assertion_' . hash('sha256', $assertionInfo['id']);
		$replayTtl = $this->replayTtl($assertionInfo['notOnOrAfter']);

		if ($cache->get($replayKey) !== null) {
			$this->traceLogger->trace('duplicate_saml_assertion_blocked', [
				'assertionId' => $assertionInfo['id'],
			]);

			// Already consumed. If this is the user's browser re-posting, their
			// session is set up already; if it's a replay, no session is minted
			// either way.
			return new RedirectResponse($this->urlGenerator->linkToRouteAbsolute('dashboard.dashboard.index'));
		}

		$cache->set($replayKey, time(), $replayTtl);

		// The AuthnRequest has now been answered — don't let its ID be reused.
		...
		// RelayState is echoed back by the IdP exactly as it was given, so it is
		// no more trustworthy here than any other request parameter: validate it
		// again rather than assuming login() was where it came from.
		$returnTo = $this->safeReturnTo(
			(string)($this->request->getParam('RelayState', '') ?: $savedRelay ?: '')
		);
	...
	/**
	 * Resolve a caller-supplied post-login destination.
	 *
	 * Anything that is not a URL on this instance is dropped in favour of the
	 * dashboard, as is anything pointing back into the SAML endpoints — those
	 * would bounce the user straight back into the login flow they just
	 * completed.
	 */
	private function safeReturnTo(?string $candidate): string {
		$dashboard = $this->urlGenerator->linkToRouteAbsolute('dashboard.dashboard.index');

		$returnTo = $this->redirectValidator->toLocalUrl($candidate, $dashboard);

		$path = parse_url($returnTo, PHP_URL_PATH) ?: '';
		if (str_contains($path, '/apps/opencase/saml')) {
			return $dashboard;
		}
	...
	/**
	 * Parse XML that arrived from outside, refusing anything malformed.
	 *
	 * Entity substitution is left off (LIBXML_NONET too) so a hostile document
	 * cannot pull in external entities — the parse must yield exactly the
	 * bytes we were sent, since that is what the signature is checked over.
	 */
	private function loadXmlStrict(string $xml, string $what): \DOMDocument {
		if (trim($xml) === '') {
			throw new SamlValidationException("Empty {$what}");
		}
		...
		// No LIBXML_NOCDATA / whitespace normalisation here: the document has to
		// canonicalise to exactly what the IdP signed, so it is parsed as-is.
		$loaded = $doc->loadXML($xml, LIBXML_NONET);
		$errors = libxml_get_errors();
		libxml_clear_errors();
		libxml_use_internal_errors($previousUseErrors);

		$fatal = array_filter($errors, static fn (\LibXMLError $e) => $e->level >= LIBXML_ERR_ERROR);

		if (!$loaded || !empty($fatal) || $doc->documentElement === null) {
			throw new SamlValidationException("Failed to parse {$what}");
		}

		// A DOCTYPE is never legitimate here and is the lead-in to entity
		// expansion attacks — reject rather than rely on parser defaults.
		foreach ($doc->childNodes as $child) {
			if ($child->nodeType === XML_DOCUMENT_TYPE_NODE) {
				throw new SamlValidationException("{$what} contains a DOCTYPE declaration");
			}
		}
	...
	/**
	 * How long a consumed assertion ID is remembered, so the same assertion
	 * cannot be presented twice while it is still inside its validity window.
	 */
	private function replayTtl(?\DateTimeImmutable $notOnOrAfter): int {
		$default = 900;
		...
		$remaining = $notOnOrAfter->getTimestamp() - time();

		// Keep it at least as long as the assertion is valid (plus the clock
		// skew the validator tolerates), and cap it so the cache can't be
		// filled by assertions claiming an absurd lifetime.
		return max($default, min($remaining + 300, 86400));
	}
	...
	public function logout(): Response {
		try {
			...
			// Defaults to, and is pinned to, this instance: the value is sent to
			// the IdP as RelayState and is a redirect target on the way back.
			$returnTo = $this->redirectValidator->toLocalUrl(
