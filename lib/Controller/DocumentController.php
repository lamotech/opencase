...
use OCA\OpenCase\Db\DocumentTypeMapper;
 ...
 *   GET    /api/v1/document-types                     — List user-selectable document types
...
class DocumentController extends ApiController {
    ...
    /**
     * List the user-selectable document types.
     *
     * System types (is_system = 1) are assigned by integrations such as Digital
     * Post and are deliberately left out so they cannot be picked by hand.
     */
    #[NoAdminRequired]
    #[ApiRoute(verb: 'GET', url: '/api/v1/document-types')]
    public function types(): DataResponse {
        return new DataResponse(['types' => $this->documentTypeMapper->findAll(true, false)]);
    }
     ...
     *   title         — Partial match against document title or document number (ILIKE)
    ...
    public function create(int $caseId): DataResponse {
        ...
        try {
            ...
            $categoryNames = $this->documentCategoryMapper->getNameMap($lang);
            return new DataResponse(
                ['document' => $this->serializeDocument($entity, $statusNames, $statusFinals, $insightNames, $categoryNames, true)],
                Http::STATUS_CREATED
            );
    ...
    public function show(int $id): DataResponse {
        try {
            ...
            $entity        = $this->documentService->get($id, $this->userId, $audit);
            $lang          = $this->getUserLanguage();
            $statusNames   = $this->documentStatusMapper->getNameMap($lang);
            $statusFinals  = $this->documentStatusMapper->getIsFinalMap();
            $ilId          = $entity->getInsightLevelId();
            $insightNames  = $ilId ? $this->insightLevelMapper->getNamesByIds([$ilId], $lang) : [];
            $categoryNames = $this->documentCategoryMapper->getNameMap($lang);
            $canWrite      = $this->documentService->canWrite($id, $this->userId);
            return new DataResponse(['document' => $this->serializeDocument($entity, $statusNames, $statusFinals, $insightNames, $categoryNames, $canWrite)]);
        } catch (NotFoundException $e) {
            return new DataResponse(
     ...
     */
    private function serializeDocument($entity, array $statusNames = [], array $statusFinals = [], array $insightNames = [], array $categoryNames = [], bool $canWrite = false): array {
