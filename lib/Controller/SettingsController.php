...
class SettingsController extends Controller {
    ...
    public function downloadMsofficeManifest(string $app): DataDownloadResponse {
        ...
        $opencaseHost = $this->resolveOpencaseHost();
        $xml = str_replace(
            ['ADDIN_HOST', 'OPENCASE_HOST'],
            [$this->resolveAddinHost($opencaseHost), $opencaseHost],
     ...
     * placeholder in the MS Office manifest templates, used for
     * <AppDomains> since the login dialog navigates there. Mirrors
     * msoffice/nginx/deploy.sh's detect_opencase_host(): overwrite.cli.url,
     * falling back to the first trusted domain, falling back to the host
     * of the current request.
     */
    private function resolveOpencaseHost(): string {
        $host = '';
     ...
     */
    private function resolveAddinHost(string $opencaseHost): string {
    ...
    /**
     * Search the transaction log.
     *
     * Request body: { "username": "...", "from": "YYYY-MM-DD HH:MM:SS", "to": "...", "type": "..." }
    ...
    public function searchTransactionLog(): JSONResponse {
        ...
        foreach ($rows as &$row) {
            ...
            $row['display_name'] = $user ? $user->getDisplayName() : $userId;
        }
        unset($row);
    ...
    public function setApiClientActive(int $id): JSONResponse {
        ...
        return new JSONResponse(['clients' => $this->apiClientRepository->findAll()]);
    }
