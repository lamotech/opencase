...
class OpenCaseDocumentsSearchProvider implements IProvider {
    ...
    public function search(IUser $user, ISearchQuery $query): SearchResult {
        ...
        $directCaseIds = $this->permissionService->getDirectAccessCaseIds($user->getUID());
        if (empty($profileIds) && empty($directCaseIds)) {
            return SearchResult::paginated($this->getName(), [], $offset + $limit);
        }

        $rows = $this->documentMapper->searchByTitle($term, $profileIds, $limit, $offset, $directCaseIds);
        $entries = [];
