...
class AiPromptMapper extends QBMapper {
    ...
    /**
     * Return the prompts matching the given ids whose scope matches one of the given scopes,
     * ordered by title. Used to resolve a user's favorited prompt ids into full prompts.
     *
     * @param int[] $ids
     * @param string[] $scopes
     * @return AiPrompt[]
     */
    public function findByIds(array $ids, array $scopes): array {
        if (empty($ids)) {
            return [];
        }
        $qb = $this->db->getQueryBuilder();
        $qb->select('*')
            ...
            ->andWhere($qb->expr()->in('scope', $qb->createNamedParameter($scopes, IQueryBuilder::PARAM_STR_ARRAY)))
            ->orderBy('title', 'ASC');
        return $this->findEntities($qb);
    }
