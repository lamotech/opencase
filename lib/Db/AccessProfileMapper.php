...
class AccessProfileMapper extends QBMapper {
    ...
    /**
     * Get distinct organisations (uuid + name) that appear in the given
     * profile IDs. Joins with opencase_org to return uuid alongside the
     * human-readable name — needed by callers that go on to reference the
     * organisation by uuid (e.g. creating a case).
     *
     * @param int[] $profileIds
     * @return list<array{org_uuid: string, org_name: string}>
     */
    public function getDistinctOrganisationsWithUuid(array $profileIds): array {
        if (empty($profileIds)) {
            return [];
        }
        ...
        $qb    = $this->db->getQueryBuilder();
        $nameFn = $qb->createFunction('o.org_name');
        $qb->select('o.org_uuid', 'o.org_name')
            ->from($this->getTableName(), 'ap')
            ->innerJoin('ap', 'opencase_org', 'o', $qb->expr()->eq('ap.org_uuid', 'o.org_uuid'))
            ->where($qb->expr()->in(
                'ap.id',
            ...
            ->groupBy('o.org_uuid', $nameFn)
            ->orderBy($nameFn, 'ASC');

        $result = $qb->executeQuery();
        $orgs   = [];
        while ($row = $result->fetch()) {
            $orgs[] = ['org_uuid' => $row['org_uuid'], 'org_name' => $row['org_name']];
        }
        $result->closeCursor();
