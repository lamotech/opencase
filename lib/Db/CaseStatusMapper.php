...
/**
 * Mapper for oc_opencase_casestatus.
 *
 * The table has a composite PK (id, language) — one display-name row per
 * status per language. Numeric IDs are shared across languages.
 *
 * @extends QBMapper<CaseStatus>
 */
class CaseStatusMapper extends QBMapper {

    public function __construct(IDBConnection $db) {
        parent::__construct($db, 'opencase_casestatus', CaseStatus::class);
    }

    /**
     * Return all statuses for the given language.
    ...
    /**
     * Return a [statusId => ['name' => ..., 'is_closed' => bool]] map
     * for the given IDs in the given language (falls back to $fallback).
     *
     * @param int[]  $ids
     * @return array<int, array{name: string, is_closed: bool}>
     */
    ...
    private function queryForLanguage(string $language, bool $excludeExpired = false): array {
        ...
        $qb->select('id', 'language', 'name', 'is_closed', 'expired')
