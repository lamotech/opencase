...
/**
 * Mapper for oc_opencase_insightlevel.
 *
 * The table has a composite PK (id, language) — one display-name row per
 * insight level per language. Numeric IDs are shared across languages.
 *
 * @extends QBMapper<InsightLevel>
 */
class InsightLevelMapper extends QBMapper {

    public function __construct(IDBConnection $db) {
        parent::__construct($db, 'opencase_insightlevel', InsightLevel::class);
    }

    /**
     * Return all insight levels for the given language.
    ...
    /**
     * Return a [id => name] map for the given IDs in the given language
    ...
    private function queryForLanguage(string $language, bool $excludeExpired = false): array {
        ...
        $qb->select('id', 'language', 'name', 'description', 'expired')
