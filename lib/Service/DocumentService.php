...
class DocumentService {
     ...
     */
    public function get(int $documentId, string $userId): Document {
        $entity = $this->documentMapper->findById($documentId);
        if ($entity === null) {
            throw new NotFoundException("Document {$documentId} not found");
        }
    ...
    public function listInboxDocuments(string $userId, int $limit = 50, int $offset = 0): array {
        ...
        $inboxCaseIds = $this->caseMapper->findInboxCaseIdsForUser($profileIds, $userId);
