...
*/
     ...
     */
    public function getAccessibleOrganisationsWithUuid(array $accessProfileIds): array {
        if (empty($accessProfileIds)) {
            return [];
        }
        return $this->accessProfileMapper->getDistinctOrganisationsWithUuid($accessProfileIds);
    }
    ...
    public function userHasWriteAccessToDocument(string $userId, int $documentId): bool {
        ...
        $caseId = $this->getCaseIdForDocument($documentId);
        if ($caseId !== null && $this->userHasWriteAccessToCase($userId, $caseId)) {
            return true;
        }

        // Document-level file share recipients with write (UPDATE) permission
        return $this->userHasWriteFileShareForDocument($userId, $documentId);
    }

    /**
     * Return true if the user has at least one active file share with the
     * UPDATE permission bit for any file belonging to this document.
     */
    private function userHasWriteFileShareForDocument(string $userId, int $documentId): bool {
        $now = (new \DateTime())->format('Y-m-d H:i:s');
        $qb  = $this->db->getQueryBuilder();
        $qb->select('fs.permissions')
            ->from('opencase_file_shares', 'fs')
            ->innerJoin('fs', 'opencase_files', 'f', $qb->expr()->eq('fs.file_id', 'f.id'))
            ->where($qb->expr()->eq('f.document_id', $qb->createNamedParameter($documentId, $qb::PARAM_INT)))
            ->andWhere($qb->expr()->eq('fs.shared_with', $qb->createNamedParameter($userId)))
        ...
        $result = $qb->executeQuery();
        while (($permissions = $result->fetchOne()) !== false) {
            if (((int)$permissions) & \OCP\Constants::PERMISSION_UPDATE) {
                $result->closeCursor();
                return true;
            }
        }
    ...
    /**
     * Return true if the user can reach any case at all, by any path
     * (access profile, responsible user, or active direct grant).
     *
     * Used as a cheap gate in front of the document search queries. Those
     * scope themselves with correlated EXISTS subqueries, which is the right
     * shape for a user who has access — but for a user who has none, the
     * optimiser would still walk opencase_documents looking for rows that can
     * never match. Three indexed LIMIT 1 probes rule that out up front; the
     * profile lookup is served from cache on the common path.
     */
    public function userHasAnyAccess(string $userId): bool {
        if (!empty($this->getAccessProfileIdsForUser($userId))) {
            return true;
        }

        return $this->userHasAnyCaseAccess($userId);
    }
