...
class ElasticsearchService {
    ...
    /**
     * Whether the configured Elasticsearch cluster answers right now.
     *
     * Uses a short timeout so callers that run in an interactive context
     * (the install repair step) are not blocked for the full request timeout
     * when es_host points at a host that does not exist yet.
     */
    public function isAvailable(): bool {
        try {
            $this->esRequest('GET', '/', null, 5);
            return true;
        } catch (\Throwable $e) {
            return false;
        }
        ...
        $indexBody = [
            ...
            'mappings' => [
                'properties' => [
                    ...
                    'year' => ['type' => 'keyword'],
                    // Case type is indexed by id, not by name: names are
                    // localised per user language, so an id keeps the filter
                    // language-independent. The UI resolves ids to labels.
                    'casetype_id' => ['type' => 'integer'],
                    'size' => ['type' => 'long'],
    ...
    public function indexDocument(int $fileId, array $document): void {
        $this->withIndexSetup(fn() => $this->esRequest(
            'PUT',
    ...
    /**
     * Run an indexing request, creating the index and ingest pipeline on the
     * fly if they are missing.
     *
     * Without this, a cluster that has never had setupIndex() run against it
     * rejects every write (404 on the index, 400 on the unknown pipeline) and
     * the failure is only visible in the log — searches then silently return
     * no files.
     */
    private function withIndexSetup(callable $request): void {
        try {
            $request();
            return;
        } catch (\Exception $e) {
            $this->logger->info('OpenCase ES: index write failed, running setup and retrying: {error}', [
                'error' => $e->getMessage(),
            ]);
        }
    ...
    public function storeDocument(int $fileId, array $document): void {
        $this->withIndexSetup(fn() => $this->esRequest('PUT', "/{$this->index}/_doc/{$fileId}", $document));
    }
    ...
    public function search(
        ...
        if (!empty($filters['case_type'])) {
            $filterClauses[] = ['term' => ['casetype_id' => (int)$filters['case_type']]];
        }
    ...
    private function esRequest(string $method, string $path, ?array $body = null, int $timeout = 30): array {
