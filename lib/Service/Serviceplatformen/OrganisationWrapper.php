 ...
 */
class OrganisationWrapper extends ServiceplatformenWrapper
{
    private const SOAP_ACTION_FREMSOEG = 'http://kombit.dk/sts/organisation/organisationssystem/fremsoegobjekthierarki';
    private const SOAP_ACTION_LAES = 'http://kombit.dk/sts/organisation/organisationenhed/laes';
    ...
    public function __construct(OrganisationConfiguration $configuration, SAMLToken $token)
    {
        ...
        $this->currentAction = self::SOAP_ACTION_FREMSOEG;
    }
    ...
    {
        $this->currentAction = self::SOAP_ACTION_LAES;
        $transactionUUID = $this->generateUUID();
        $messageId = 'urn:uuid:' . $this->generateUUID();
        ...
        try {
            $soapEnvelope = $this->buildSignedSoapEnvelopeForLaes($transactionUUID, $messageId, $UUIDIdentifikator, $pemFiles);
            $response = $this->sendRequest($soapEnvelope, $pemFiles);
...
<s:Header>
<a:Action s:mustUnderstand="1" u:Id="{$actionId}">{$this->escapeXml(self::SOAP_ACTION_FREMSOEG)}</a:Action>
...
XML;
    ...
    private function extractLaesOutput(string $soapResponse): string
    {
        ...
        $outputNodes = $xpath->query('//ns5:LaesOutput');
        if ($outputNodes->length === 0) {
    ...
    private function buildSignedSoapEnvelopeForLaes(string $transactionUUID, string $messageId, string $UUIDIdentifikator, array $pemFiles): string
    {
        $endpoint = $this->configuration->getEndpoint();
        $samlAssertion = $this->samlToken->getAssertion();
...
<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:a="http://www.w3.org/2005/08/addressing" xmlns:u="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd">
<s:Header>
<a:Action s:mustUnderstand="1" u:Id="{$actionId}">{$this->escapeXml(self::SOAP_ACTION_LAES)}</a:Action>
<h:RequestHeader xmlns:h="http://kombit.dk/xml/schemas/RequestHeader/1/" xmlns="http://kombit.dk/xml/schemas/RequestHeader/1/"><TransactionUUID>{$this->escapeXml($transactionUUID)}</TransactionUUID></h:RequestHeader>
<a:MessageID u:Id="{$messageIdId}">{$this->escapeXml($messageId)}</a:MessageID>
<a:ReplyTo u:Id="{$replyToId}"><a:Address>http://www.w3.org/2005/08/addressing/anonymous</a:Address></a:ReplyTo>
<a:To s:mustUnderstand="1" u:Id="{$toId}">{$this->escapeXml($endpoint)}</a:To>
