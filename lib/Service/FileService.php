...
class FileService {
    ...
    /**
     * Download a file without user permission checks.
     * For internal server-side use only (e.g. export generation).
     */
    public function downloadInternal(int $fileId): array {
        $entity = $this->fileMapper->findById($fileId);
        if ($entity === null) {
            throw new NotFoundException("File {$fileId} not found");
        }
     ...
     *
     * Both ends are checked, before anything else happens: reading the source
     * document's files requires read access to it, and putting copies into the
     * target requires write access there. Without the source check this is a
     * way to lift files out of any case in the system — copy them into a
     * document you own, then download them from there.
     *
     * @return FileEntity[] The newly created file entities
     * @throws NotFoundException if either document is unreachable for this user
     */
    public function copyFilesToDocument(int $sourceDocumentId, int $targetDocumentId, string $userId): array {
        ...
        if (!$this->permissionService->userHasReadAccessToDocument($userId, $sourceDocumentId)) {
            throw new NotFoundException('Access denied');
        }

        if (!$this->permissionService->userHasWriteAccessToDocument($userId, $targetDocumentId)) {
            throw new NotFoundException('Access denied');
        }
