...
class CaseService {
    ...
    public function create(
        ...
        // Verify user has write access to this profile
        if (!$this->permissionService->getUserAccessLevel($userId, $profile->getId()) === 'write') {
     ...
     */
    public function get(int $caseId, string $userId): CaseEntity {
        $entity = $this->caseMapper->findById($caseId);
        if ($entity === null) {
            throw new NotFoundException("Case {$caseId} not found");
        }
    ...
    public function list(
        ...
        // profileIds may be empty (user has no profile assignments) — that is fine,
        // the mapper will still surface cases via responsible_user_id and direct grants.
        ...
        if ($search !== null && trim($search) !== ''
            ...
            $cases = $this->caseMapper->searchByTitleOrNumber($search, $profileIds, $userId, $limit);
        ...
        $total = $this->caseMapper->countWithFilters(
            $profileIds, $userId, $orgUuid, $year, $statusId,
        ...
        $cases = $this->caseMapper->findWithFilters(
            $profileIds, $userId, $orgUuid, $year, $statusId, $limit, $offset,
            $search, $classificationFacetUuid, $insightLevelId, $responsibleUserId,
            $classificationCode, $sensitivityKey, $casetypeId,
        );
    ...
    public function getStats(string $userId): array {
        $profileIds = $this->permissionService->getAccessProfileIdsForUser($userId);
        return $this->caseMapper->countByOrgAndYear($profileIds);
    }
