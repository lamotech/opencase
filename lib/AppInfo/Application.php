...
class Application extends App implements IBootstrap {
    ...
    public function __construct(array $urlParams = []) {
        ...
        file_put_contents('/tmp/opencase_app_boot.txt', date('c') . " Application constructed\n", FILE_APPEND);
    }
    ...
    public function boot(IBootContext $context): void {
		...
		if (!$jobList->has(IndexFileJob::class, null)) {
			$jobList->add(IndexFileJob::class);
		}
		if (!$jobList->has(BulkReindexJob::class, null)) {
			$jobList->add(BulkReindexJob::class);
		}
