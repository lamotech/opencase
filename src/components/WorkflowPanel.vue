...
export default {
	...
	mounted() {
		this._aiHandler = (documentId) => {
			if (String(documentId) === String(this.documentId)) this.load()
		}
		eventBus.on('ai:workflow-changed', this._aiHandler)
	},

	beforeUnmount() {
		eventBus.off('ai:workflow-changed', this._aiHandler)
	},
