...
import { maskedPartyNumber } from '../utils/cprMask.js'
...
const ACCESS_STATUS_LABELS = {
	...
	sent:       'Afsendt',
	rejected:   'Afvist',
	partly_rejected: 'Delvist afvist',
}

const ACCESS_DECISION_LABELS = {
	full_access:     'Fuld aktindsigt',
	partial_access:  'Delvis aktindsigt',
	rejected:        'Afslag',
...
export default {
	...
	methods: {
		...
		eventIcon(eventType) {
			...
			case 'access_request_created':       return FileEyeOutlineIcon
			case 'access_request_status_changed': return SwapHorizontalIcon
			case 'access_request_updated':       return PencilIcon
			case 'access_request_decision':      return GavelIcon
			case 'access_request_sent':          return SendIcon
		...
		describe(entry) {
			...
			case 'case_metadata_changed': {
				...
				if (d.summary) {
					parts.push(t('opencase', 'Sagsresume ændret'))
				}
			...
			case 'case_participant_added':
				return t('opencase', 'Part tilføjet: {role} – {name} ({cprCvr})', {
					role: d.role || '',
					name: d.name || '',
					cprCvr: maskedPartyNumber(d),
				})
			case 'case_participant_deleted':
				return t('opencase', 'Part slettet: {role} – {name} ({cprCvr})', {
					role: d.role || '',
					name: d.name || '',
					cprCvr: maskedPartyNumber(d),
				})
			...
			case 'access_request_sent': {
				...
				return t('opencase', 'Aktindsigt afsendt ({method})', { method })
			}
