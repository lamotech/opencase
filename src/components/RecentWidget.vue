<template>
	<div class="opencase-widget">
		...
		<p v-else class="opencase-widget__empty">
			{{ t('opencase', 'Ingen seneste aktiviteter endnu') }}
		</p>

		<div class="opencase-widget__footer">
			<a :href="allRecentUrl" class="opencase-widget__view-all">
				{{ t('opencase', 'Vis alle') }}
			</a>
		</div>
	</div>
...
export default {
	...
	computed: {
		allRecentUrl() {
			return generateUrl('/apps/opencase/recent')
		},
	},
	...
	methods: {
		async fetchRecent() {
			const url = generateOcsUrl('/apps/opencase/api/v1') + '/widget/recent'
			try {
				...
				console.error('[OpenCase recent widget] failed', e?.response?.status, e?.message)
			} finally {
				this.loading = false
			}
		},
