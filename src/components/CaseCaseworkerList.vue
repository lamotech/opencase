...
export default {
	...
	methods: {
		...
		async addCaseworker() {
			...
			if (!this.caseId) {
				if (this.caseworkers.some(c => c.user_id === this.selectedUser.id)) {
					this.addError = t('opencase', 'Denne bruger er allerede tilføjet')
					return
				}
				this.caseworkers.push({ user_id: this.selectedUser.id, display_name: this.selectedUser.label, role: 'other' })
				this.$emit('count-changed', this.caseworkers.filter(c => c.role !== 'responsible').length)
