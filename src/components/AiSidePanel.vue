...
export default {
	...
	computed: {
		currentCaseId() {
			if (this.$route.name === 'case-detail') return this.$route.params.id
			if (this.$route.name === 'document-detail') return this.$store.state.currentCase?.id ?? null
			return null
		},
	...
	methods: {
		...
		async onPromptExecuteActions(actions, label) {
			...
			try {
				await this.runActionChain(actions, contextSnapshot, runtimeCtx, msgEntry)
			} catch (err) {
		...
		async send() {
			...
			try {
				...
				// IDs produced by earlier ones (e.g. currentCaseId from create_case_and_move_document
		...
		// Runs an action chain in order, stopping early if a case/document-creating action
		// fails. Without this, a failed create_case would leave runtimeCtx.newCaseId unset,
		// and later actions (e.g. add_company_participant) would silently fall back to
		// whatever case/document happens to be open in the UI instead of aborting.
		async runActionChain(actions, ctx, runtimeCtx, msgEntry) {
			const abortOnFailure = new Set(['create_case', 'create_case_and_move_document', 'create_document'])
			for (const action of (actions ?? [])) {
				const result = await this.executeAction(action, ctx, runtimeCtx)
				msgEntry.actionResults.push(result)
				if (!result.ok && abortOnFailure.has(action.type)) break
			}
		},
		...
		async executeAction(action, ctx, runtimeCtx = {}) {
			try {
				...
				case 'add_sender_as_participant':
					return await this.actionAddSenderAsParticipant(action, ctx, runtimeCtx)
				...
				case 'create_workflow':
					return await this.actionCreateWorkflow(action, ctx, runtimeCtx)
				default:
		...
		async _buildAndCreateCase(action) {
			...
			// "#MinAfdeling" is a macro meaning "the user's own primary organisation" —
			// treat it the same as an omitted organisation instead of looking it up by name.
			if (orgName.toLowerCase() === '#minafdeling') orgName = ''
			if (!orgName) orgName = await api.getMyPrimaryOrg()
			if (!orgName) throw new Error(t('opencase', 'Ingen organisation fundet.'))

			const [sensitivities, insightLevels, facets, caseTypes] = await Promise.all([
			...
			const wantedCaseType = (action.casetype ?? action.case_type ?? '').trim().toLowerCase()
			let caseType = null
			if (wantedCaseType) {
				caseType = caseTypes.find(c => c.name?.toLowerCase() === wantedCaseType)
					?? caseTypes.find(c => c.name?.toLowerCase().includes(wantedCaseType))
				if (!caseType) throw new Error(t('opencase', 'Sagstypen \'{c}\' blev ikke fundet.', { c: action.casetype ?? action.case_type }))
			}
		...
		async actionCreateCase(action, ctx, runtimeCtx = {}) {
			try {
				...
				runtimeCtx.currentCaseId = created.id
		...
		async actionCreateDocument(action, ctx, runtimeCtx = {}) {
			...
			if (action.template) {
				...
				if (template) {
					...
					eventBus.emit('ai:files-changed', doc.id)
				} else {
		...
		async actionAddFileFromTemplate(action, ctx, runtimeCtx = {}) {
			...
			const createdFile = await api.createFileFromTemplate(docId, template.id)
			eventBus.emit('ai:files-changed', docId)

			let label = t('opencase', 'Fil oprettet fra skabelon \'{s}\'.', { s: template.name })

			// Optionally open the created file for editing in Office
			if (action.open_in_office) {
				try {
					const ncFileId = await api.getEditUrl(createdFile.id, 'edit')
					window.open(generateUrl('/f/' + ncFileId), '_blank')
				} catch {
					label += ' ' + t('opencase', 'Filen blev oprettet, men kunne ikke åbnes i Office.')
				}
			}
		...
		async actionAddCitizenContact(action, ctx, runtimeCtx = {}) {
			...
			if (!warning) eventBus.emit('ai:contacts-changed', docId)
			return warning
		...
		async actionMoveToCase(action, ctx, runtimeCtx = {}) {
			...
			eventBus.emit('ai:document-moved', doc)
		...
		async actionAddSenderAsReceiver(action, ctx, runtimeCtx = {}) {
			...
			eventBus.emit('ai:contacts-changed', targetDocId)
		...
		async actionAddSenderAsParticipant(action, ctx, runtimeCtx = {}) {
			// Source: the original inbox document; target: the case (current, or the one
			// just created by create_case_and_move_document earlier in this chain).
			const sourceDocId = runtimeCtx.sourceDocumentId ?? ctx?.document_id ?? this.currentDocumentId
			if (!sourceDocId) {
				return { ok: false, label: t('opencase', 'Intet kildedokument at kopiere afsender fra.') }
			}
			...
			const [contacts, contactRoles, participantRoles] = await Promise.all([
				api.getDocumentContacts(sourceDocId),
				api.getContactRoles(),
				api.getParticipantRoles(),
			])

			const senderRole = contactRoles.find(r => r.name?.toLowerCase().includes('afsend'))
			const senders = senderRole
				? contacts.filter(c => c.contactrole_id === senderRole.id)
				: contacts
			if (!senders.length) {
			...
			const wanted = (action.participant_role ?? 'part').toLowerCase()
			const participantRole = participantRoles.find(r => r.name?.toLowerCase() === wanted)
				?? participantRoles.find(r => r.name?.toLowerCase().includes(wanted))
				?? participantRoles[0]
			if (!participantRole) {
				return { ok: false, label: t('opencase', 'Ingen partroller fundet.') }
			}
			...
			for (const sender of senders) {
				...
				if (cprCvr.length === 10) {
					const results = await api.searchCitizen({ cpr: cprCvr }).catch(() => [])
					entity = results[0] ? { ...results[0], cpr_cvr: results[0].cpr_cvr ?? sender.cpr_cvr } : null
				} else if (cprCvr.length === 8) {
					const results = await api.searchCompany({ cvr: cprCvr }).catch(() => [])
					entity = results[0] ? { ...results[0], cpr_cvr: results[0].cpr_cvr ?? sender.cpr_cvr } : null
				}
			...
			if (!added) {
				return { ok: false, label: t('opencase', 'Afsenderen kunne ikke findes som borger eller virksomhed.') }
			}

			eventBus.emit('ai:participants-changed', caseId)
			return { ok: true, label: t('opencase', '{n} afsender(e) tilføjet som part på sagen.', { n: added }) }
		},
		...
		async actionAddEmployeeParticipant(action, ctx, runtimeCtx = {}) {
			...
			const warning = await this.addEmployeeParticipant(caseId, action.employee ?? action.user_id, action.participant_role)
			if (!warning) eventBus.emit('ai:participants-changed', caseId)
			return warning
				? { ok: false, label: warning }
				: { ok: true, label: t('opencase', 'Medarbejder tilføjet som part på sagen.') }
		},

		// Employee participants are Nextcloud users (looked up in oc_opencase_userinfo by
		// username, personname or Nextcloud user-id) rather than citizens/companies, so they
		// carry a user_id instead of a cpr_cvr — the backend derives contact_type EMPLOYEE
		// automatically when cpr_cvr is left empty.
		async addEmployeeParticipant(caseId, query, roleName) {
			const q = (query ?? '').trim()
			if (!q) return t('opencase', 'Brugernavn, navn eller bruger-id er påkrævet.')

			const employees = await api.searchEmployees({ q })
			if (!employees.length) return t('opencase', 'Medarbejder \'{q}\' blev ikke fundet.', { q })
		...
		async actionCreateJournalNote(action, ctx, runtimeCtx = {}) {
			...
			eventBus.emit('ai:journal-notes-changed', caseId)
		...
		async actionAddOtherCaseworker(action, ctx, runtimeCtx = {}) {
			...
			eventBus.emit('ai:caseworkers-changed', caseId)
		...
		async actionGrantAccess(action, ctx, runtimeCtx = {}) {
			...
			eventBus.emit('ai:grants-changed', caseId)
		...
		async actionCreateSubCase(action, ctx, runtimeCtx = {}) {
			...
			// currentCaseId (not newCaseId) so actions following this one in the chain — e.g.
			// add_employee_participant, create_journalnote — target this sub-case. newCaseId stays
			// pointed at the parent so a subsequent create_sub_case still uses it, not this sub-case.
			runtimeCtx.currentCaseId = created.id
			...
			if (action.employee) {
				const w = await this.addEmployeeParticipant(created.id, action.employee, action.participant_role)
				if (w) warnings.push(w)
			}
		...
		async actionCreateNote(action, ctx, runtimeCtx = {}) {
			...
			eventBus.emit('ai:notes-changed', docId)
		...
		async actionCreateWorkflow(action, ctx, runtimeCtx = {}) {
			const docId = runtimeCtx.lastCreatedDocumentId ?? ctx?.document_id ?? this.currentDocumentId
			if (!docId) {
				return { ok: false, label: t('opencase', 'Intet dokument er åbent.') }
			}

			const typeRaw = (action.workflow_type ?? action.type ?? '').trim().toLowerCase()
			const type = typeRaw.startsWith('godkend') || typeRaw === 'approval' ? 'approval'
				: typeRaw.startsWith('gennem') || typeRaw === 'review' ? 'review'
				: null
			if (!type) {
				return { ok: false, label: t('opencase', 'Ukendt workflow-type \'{t}\'. Brug \'Gennemse\' eller \'Godkendelse\'.', { t: action.workflow_type ?? '' }) }
			}

			const names = String(action.users ?? '').split(',').map(s => s.trim()).filter(Boolean)
			if (!names.length) {
				return { ok: false, label: t('opencase', 'Mindst én bruger er påkrævet til workflowet.') }
			}

			const userIds = []
			const notFound = []
			for (const name of names) {
				const userId = await this._resolveEmployeeUserId(name)
				if (userId) userIds.push(userId)
				else notFound.push(name)
			}
			if (!userIds.length) {
				return { ok: false, label: t('opencase', 'Ingen af de angivne brugere blev fundet: {n}', { n: notFound.join(', ') }) }
			}

			// deadline_days ("frist i dag plus N dage") is computed client-side rather than
			// asking the model for an absolute date, since date arithmetic is unreliable to
			// leave to the model — it only needs to extract the integer N from the prompt.
			let deadline = null
			const days = Number(action.deadline_days)
			...
			try {
				await api.createWorkflow(docId, { type, deadline, user_ids: userIds })
			} catch (err) {
				const msg = err?.response?.data?.ocs?.data?.message ?? ''
				throw new Error(msg || err.message)
			}

			eventBus.emit('ai:workflow-changed', docId)

			const typeLabel = type === 'approval' ? t('opencase', 'Godkendelse') : t('opencase', 'Gennemse')
			const label = notFound.length
				? t('opencase', '{t}-workflow oprettet. Kunne ikke finde: {n}', { t: typeLabel, n: notFound.join(', ') })
				: t('opencase', '{t}-workflow oprettet.', { t: typeLabel })
			return { ok: true, label }
		},
		...
		// Resolves a name/username against oc_opencase_userinfo, preferring an exact username
		// match over an exact personname match (falls back to the first fuzzy hit). Returns the
		// linked Nextcloud user-id, or null if there's no match or the matched employee has no
		// linked Nextcloud account.
		async _resolveEmployeeUserId(query) {
			const q = (query ?? '').trim()
			if (!q) return null
			const results = await api.searchEmployees({ q }).catch(() => [])
			if (!results.length) return null
			const match = results.find(e => e.username?.toLowerCase() === q.toLowerCase())
				?? results.find(e => e.personname?.toLowerCase() === q.toLowerCase())
				?? results[0]
